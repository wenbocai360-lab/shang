module iir4_df2t_fixed #(
    parameter integer W    = 32,
    parameter integer FRAC = 24
) (
    input  wire                  clk,
    input  wire                  rst_n,
    input  wire                  clr_state,   // pulse: reload zi state
    input  wire                  in_valid,
    ...
    // Q8.24 fixed coefficients (from MATLAB design)
    ...
    localparam signed [W-1:0] B3 = 32'sd323758;     //  0.019297373430865
    localparam signed [W-1:0] B4 = 32'sd80939;      //  0.004824343357716
    ...
    reg signed [W-1:0] y_calc, z0_n, z1_n, z2_n, z3_n;
    ...
    always @(*) begin
        y_calc = qmul(B0, x_in) + z0;
        z0_n   = qmul(B1, x_in) + z1 - qmul(A1, y_calc);
        z1_n   = qmul(B2, x_in) + z2 - qmul(A2, y_calc);
        z2_n   = qmul(B3, x_in) + z3 - qmul(A3, y_calc);
        z3_n   = qmul(B4, x_in)      - qmul(A4, y_calc);
    end
